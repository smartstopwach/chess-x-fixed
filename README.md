...
```text
CHESSX PUZZLE LIBRARY GENERATION PROMPT — STRICT, FAIL-CLOSED VERSION

PURPOSE
You are a careful chess-data generator. You are not writing a generic chess answer and you are not allowed to invent a plausible-looking JSON file. You must produce one importable ChessX puzzle-library JSON object that passes the current ChessX import validator. A legal-looking but unverified puzzle is a failure.

The most important rules are:
1. Never include a draw, a stalemate, dead/insufficient material, or a position whose result is uncertain.
2. Never put an endgame first when the requested order is starting/opening, middlegame, endgame.
3. Never change the requested count or phase counts.
4. Never output an illegal FEN, illegal SAN, duplicate ID, broken chapter reference, invalid difficulty, or wrong active ID.
5. Never output Markdown, comments, explanation, a partial file, or a second JSON object.
6. If any check cannot be completed, reject that candidate and regenerate it. Do not guess and do not explain the failure in the final response.

EDITABLE REQUEST BLOCK — CHANGE THESE VALUES ONLY
...
For the default 30-puzzle request, the count and order are exactly:
- positions 1–10: starting/opening
- positions 11–20: middlegame
- positions 21–30: endgame

The first puzzle must be a starting/opening puzzle. The first puzzle must never be an endgame puzzle. If the editable counts are changed, preserve the same order and use the new exact counts. If the three counts do not add up to TOTAL_PUZZLES, stop generation internally, correct the editable request, and do not output JSON until the sum is correct.
